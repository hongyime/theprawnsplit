...
Progress:
- 2026-08-30: Privacy scrub: changed first-run name placeholder from a personal name to `e.g. John Smith` and updated the UI regression test. Commit `177dcce` pushed to main. Verification: no remaining `Bryan`/old-placeholder matches in source search; `npx vitest run test\common-expense-ui.test.ts` passed 2/2; TypeScript clean; svelte-check 0 errors/0 warnings; money lint exit 0; Vite build `✓ built in 31.24s`. Aggregate `npm run build` again stalled in root Vitest startup without emitted failures after core 81/81, matching the known Windows worker issue. Production deployment `dpl_BTVQN8AjdqU8pmaZNiSjvgE6waf2` READY; live setup placeholder is only `e.g. John Smith`, old placeholder absent, browser logs clean.
- 2026-08-30: Adjusted dashboard layout per user request to a desktop 3-column, 2-row grid: row 1 People / Balances / Settle, row 2 Ledger spanning the first two columns / Add Expense on the right; mobile stacks People, Balances, Settle, Ledger, Add Expense. Commit `b9ffa90` pushed to main. Local verification: core tests 81/81; Add Expense UI workflow 1/1; TypeScript clean; svelte-check 0 errors/0 warnings; money lint exit 0; Vite build `✓ built in 1m 18s`; local Edge smoke confirmed grid areas `"people balances settle" "ledger ledger expense"` with no console/page errors. Aggregate root Vitest and grouped UI Vitest stalled at Windows worker startup without test failures, so affected UI file was run directly. Production deployment `dpl_3mPhfbT7QhxCungsiMhr49tP2A4L` READY at `https://theprawnsplit.hong-yi.me`; live site matched "Prawn Split", live relay GET returned `{"entries":[]}`, and live Edge smoke confirmed grid areas `"people balances settle" "ledger ledger expense"` with no console/page errors.
- 2026-08-30: UI refinement requested after production smoke: restored/kept QR header action, added Copy Link -> Copied transient feedback, moved Import to a top header panel, removed header currency selector, kept per-expense currency selector, restored reachable Archive/Unarchive action, changed main ledger layout to People/Add Expense row then Balances/Settle row with Ledger below, and converted visible app copy/labels toward Proper Case. Commit `6a85aca` pushed to main. Final rebased local verification passed: `npm run build` exit 0 (core 81/81, root 203/203, svelte-check 0/0, Vite `✓ built in 55.03s`); desktop Edge smoke on built preview passed first setup, first expense, Copy Link -> Copied, QR modal, Import panel, no header currency, grid `"people expense" "balances settle"`. Production deploy `dpl_7DdD7P2d7CfCpT6LpG1imZxtis1J` READY at `https://theprawnsplit.hong-yi.me`; remote build passed core 81/81, root 203/203, svelte-check 0/0, Vite `✓ built in 2.66s`; live Edge smoke passed the same first-expense/Copy/QR/Import path; live site matches "Prawn Split"; live relay GET returns `{"entries":[]}`.
- 2026-08-30: Completed and deployed UX/add-expense pass. Root cause of cannot-add-expense was hidden Svelte dependencies in no-arg `buildSharePreview()`; `sharePreview` now receives amount, participant, selection, and split inputs explicitly. Added first-run setup card that creates + claims the user's participant before showing the ledger, toast confirmations, gated validation hints, advanced sync/recovery disclosure, and all-currency dropdowns. Commit `ac7dd5d` pushed to main. Local verification passed: focused UI tests 5 files/8 tests; full `npm run build` exit 0 (core 81/81, root 203/203, svelte-check 0/0, Vite `✓ built in 23.61s`). Vercel production deployment `dpl_5Qdy2bCwUGcL4JkBZCkmRgqmafwq` READY at `https://theprawnsplit.hong-yi.me`; remote build passed core 81/81, root 203/203, svelte-check 0/0, Vite `✓ built in 2.47s`; live Playwright desktop+mobile first-expense smoke passed with no page/console errors; live relay GET returned `{"entries":[]}`.
- 2026-08-30: Fixed production loading-ledger crash reported as `Cannot access 'a' before initialization` in `index-CoLv4HZW.js`. Root cause mapped by sourcemap to `@lucide/svelte/dist/icons/refresh-ccw.svelte`; production compiled lucide icons read generated props before initialization, while tests had stubbed lucide. Fix commit `43f9c3d`: replaced lucide runtime icons with local `src/lib/Icon.svelte`, removed `@lucide/svelte`, added `mobile-web-app-capable`, and added platform guard against direct lucide imports. Local `npm run build` passed: core 81/81, root 201/201, svelte-check 0/0, Vite `✓ built in 6.55s`. Production deploy `dpl_6VLtzTaAAfEQJ7DjXNWBaM33SKbZ` READY and aliased; follow-up Git deploy `theprawnsplit-oxlo75dqe-theprawnvercel.vercel.app` also READY. Final live Playwright ledger smoke on `https://theprawnsplit.hong-yi.me`: `hasLoading=false`, ledger signals present (`People`, `Balances`, `Expense`, `Settle`), `pageErrors=[]`, `tdzErrors=[]`; one external public-Nostr network error remains non-fatal. Operated relay POST/GET smoke passed at cursor `1788056839863-0`.
- 2026-08-30: Vercel production deploy succeeded after quota reset. Deployed `dpl_54fsPQxBkobFfqBSSNruFv8DriDs` / `https://theprawnsplit-9f153megy-theprawnvercel.vercel.app`, readyState READY, aliased to `https://theprawnsplit.hong-yi.me`. Remote build passed: core Tests 81 passed (81), root Tests 200 passed (200), svelte-check 0 errors/0 warnings, Vite `✓ built in 7.10s`. Live checks passed: `https://theprawnsplit.vercel.app` and `https://theprawnsplit.hong-yi.me` both match "Prawn Split"; live relay GET returns `{"entries":[]}` instead of unconfigured error; live relay POST/GET smoke wrote `codex-relay-smoke` and read it back at cursor `1788054561523-0`.
