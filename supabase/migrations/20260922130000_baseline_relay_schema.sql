-- REL-008/T26: baseline migration capturing the already-live prawnsplit relay
-- schema (verified via Management API during B4/T25) as migration #1, since
-- no supabase/migrations/ history exists in this repository at all -- the
-- hosted schema was evidently applied by running supabase/schemas/relay.sql
-- directly. This file is a byte-for-byte semantic copy of that desired
-- schema with idempotency guards added throughout (IF NOT EXISTS / OR
-- REPLACE / ON CONFLICT DO NOTHING), so it is SAFE to apply to either:
--   (a) a fresh, empty database (creates everything from scratch), or
--   (b) the already-live hosted database (every statement no-ops on objects
--       that already exist; no ALTER, no DROP, no data mutation beyond the
--       one no-op-guarded seed row).
-- It must never be run directly against the live hosted project by an
-- agent or CI job. See bugfix.md's REL-008 entry ("T26 user-applied
-- forward-compensation instructions") for the required user-applied step
-- (supabase migration repair) that registers this file as already-applied
-- in the hosted migration history WITHOUT executing its SQL there again.
begin;

create schema if not exists prawnsplit;
revoke all on schema prawnsplit from public, anon, authenticated;
grant usage on schema prawnsplit to service_role;

create table if not exists prawnsplit.relay_control (
...
insert into prawnsplit.relay_control (singleton) values (true) on conflict (singleton) do nothing;

create table if not exists prawnsplit.relay_topics (
...
create table if not exists prawnsplit.relay_entries (
...
create index if not exists relay_entries_receipt on prawnsplit.relay_entries(tag, pg_catalog.md5(blob), author);
...
create or replace function prawnsplit.reserve_capacity(p_bytes bigint, p_entries bigint, p_groups bigint)
...
create or replace function public.prawnsplit_relay_append(p_tag text, p_commitment text, p_blob text, p_author text)
...
create or replace function public.prawnsplit_relay_read(p_tag text, p_cursor text default null,
...
create or replace function public.prawnsplit_relay_import(p_tag text, p_commitment text, p_rows jsonb)
...
create or replace function public.prawnsplit_relay_topic_info(p_tag text)
