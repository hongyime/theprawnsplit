...
Progress:
- 2026-08-30: Completed local UX/add-expense pass. Root cause of cannot-add-expense was hidden Svelte dependencies in no-arg `buildSharePreview()`; `sharePreview` now receives amount, participant, selection, and split inputs explicitly. Added first-run setup card that creates + claims the user's participant before showing the ledger, toast confirmations, gated validation hints, advanced sync/recovery disclosure, and all-currency dropdowns. Local verification passed: focused UI tests 5 files/8 tests; full `npm run build` exit 0 (core 81/81, root 203/203, svelte-check 0/0, Vite `✓ built in 23.61s`); Playwright desktop+mobile first-expense smoke passed with no page/console errors.
