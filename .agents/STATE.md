...
Progress:
- 2026-08-30: Privacy scrub: changed first-run name placeholder from a personal name to `e.g. John Smith` and updated the UI regression test. Verification: no remaining `Bryan`/old-placeholder matches in source search; `npx vitest run test\common-expense-ui.test.ts` passed 2/2; TypeScript clean; svelte-check 0 errors/0 warnings; money lint exit 0; Vite build `✓ built in 31.24s`. Aggregate `npm run build` again stalled in root Vitest startup without emitted failures after core 81/81, matching the known Windows worker issue.
